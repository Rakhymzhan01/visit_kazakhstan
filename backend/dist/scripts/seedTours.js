...
async function seedTours() {
    try {
        ...
        const adminUser = await User_1.User.findOne({ email: 'admin@visitkazakhstan.com' });
        if (!adminUser) {
        ...
        await Tour_1.Tour.deleteMany({});
        console.log('Creating categories...');
        const createdCategories = await Category_1.Category.insertMany(sampleCategories.map(cat => ({
            ...cat,
            createdAt: new Date(),
            updatedAt: new Date()
        })));
        ...
        const createdTours = await Tour_1.Tour.insertMany(toursWithAuthor);
...
exports.default = seedTours;
//# sourceMappingURL=seedTours.js.map