...
async function seedBlogs() {
    try {
        ...
        let author = await User_1.User.findOne({ role: 'ADMIN' });
        if (!author) {
            console.log('👤 No admin user found, creating default author...');
            const hashedPassword = await bcryptjs_1.default.hash('Admin123!', 12);
            author = await User_1.User.create({
        ...
        const createdPosts = await BlogPost_1.BlogPost.insertMany(blogPostsWithAuthor);
